...
void ConfigLoader::loadParams(string filename){
  ...
  try{//Loading Params
    ...
    if(camera_calibration_file_.empty() || rgb_topic_.empty() || depth_topic_.empty() || aruco_dic_.empty() || aruco_poses_file_.empty() || aruco_minimum_distance_ == 0 || aruco_marker_size_ == 0) throw 1;
